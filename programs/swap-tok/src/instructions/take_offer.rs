...
use crate::{accounts, Offer, ANCHOR_DISCRIMINATOR};
...
pub fn send_wanted_token_maker(context: &Context<TakeOffer>) -> Result<()>{
    transfer_tokens(&context.accounts.taker_token_Account_b, &context.accounts.maker_token_Account_b, &context.accounts.offer.token_b_wanted_amount, &context.accounts.token_mint_b, &context.accounts.taker, &context.accounts.token_program)
...
pub fn withdraw_and_Close_vault(context: Context<TakeOffer>) -> Result<()>{
    ...
    let seeds = &[
        ...
        &context.accounts.offer.id.to_be_bytes()[..],
